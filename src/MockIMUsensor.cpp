...
MockIMUsensor::MockIMUsensor( char* filename ){
      // Opens motor serial hardware interface i.e. UART01
   //mockIMUsensor_fd = open(filename, O_RDWR|O_NOCTTY|O_NDELAY|O_NONBLOCK);
   //if( mockIMUsensor_fd <= 0 ){
   //   DEBUG_PRINT("Serial Port not opened. fd=%d\n", mockIMUsensor_fd);
      //exit(1);
   //}
   DEBUG_PRINT("MockIMUsensor::Serial Port Opened.\n");
   MockIMUsensor::init();
}

MockIMUsensor::~MockIMUsensor(){
   DEBUG_PRINT("Close Serial Port\n");
}

int MockIMUsensor::init( void ){
   int ret = 0;
   DEBUG_PRINT("MockIMUsensor::Init() Passed.\n");
   return ret;
}


SENSOR_BUF MockIMUsensor::getDataMagn(){
   // Local Declarations
...
void MockIMUsensor::read_sensor( unsigned char data[], unsigned char addr, unsigned char reg){
	/* opens the i2c bus 1 */
	int file;
	char filename[40];
	char buf[256] = {0};
...
void MockIMUsensor::displayprompt(){
